...
const HeaderMenu = () => {
  ...
  return (
      ...
      <div className=" md:flex w-1/3 items-center gap-7 justify-between text-sm capitalize font-normal text-shop_light_green " aria-label="Main navigation">
        {headerData.map((item) => (
        <Link
        key={item?.title}
        href={item?.path}
        ...
        >
            ...
            <span className={`md:absolute -bottom-0.5 left-1/2 w-0 h-0.5 bg-shop_btn_green group-hover:w-1/2 hoverEffect group-hover:left-0 ${pathName === item?.path && 'w-1/2'} `}/>
